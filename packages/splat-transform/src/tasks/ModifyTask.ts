...
interface ISplatModify {
    ...
    indicesTransform: Array<{ indices: number[]; transform: number[] }>;
}

async function createSplatModify(path: string, counts: number) {
    if (!path) {
        return undefined;
    }
    const {
        isRowMatrix = true,
        transform,
        deletedIndices: deletedIndicesBitMap = [],
        indicesTransform = [],
    } = JSON.parse(fs.readFileSync(path, 'utf-8')) as Partial<ISplatModify>;
...
export class ModifyTask extends BaseTask<Config> {
    override async exec(config: Config, { logger, resources }: Context) {
        ...
        const modifies = await Promise.all(
            modifyPaths.map((p, i) => createSplatModify(p, splat.blockContentCounts[i])),
        );
