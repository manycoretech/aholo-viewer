...
interface BufferSlot {
    bufferA: GPUBuffer;
    bufferB: GPUBuffer;
    readTypesBuffer: GPUBuffer;
    readMasksBuffer: GPUBuffer;
    typesOutBuffer: GPUBuffer;
    masksOutBuffer: GPUBuffer;
    capacity: number;
...
function makeBuffer(device: GPUDevice, size: number, usage: number): GPUBuffer {
    return device.createBuffer({ size: Math.max(4, size), usage });
}

function writeUniform(device: GPUDevice, values: Uint32Array): GPUBuffer {
...
function createStoragePipeline(device: GPUDevice, code: string): GPUComputePipeline {
...
function blockAlignedExtent(halfExtent: number): number {
...
class GpuDilation {
    ...
    private readonly dilateXPipeline: GPUComputePipeline;
    private readonly dilateYZPipeline: GPUComputePipeline;
    private readonly slots: BufferSlot[] = [];
    private srcTypesBuffer: GPUBuffer | undefined;
    private srcKeysBuffer: GPUBuffer | undefined;
    private srcLoBuffer: GPUBuffer | undefined;
    private srcHiBuffer: GPUBuffer | undefined;
    ...
    constructor(device: GPUDevice) {
        ...
        for (let i = 0; i < GpuDilation.NUM_SLOTS; i++) {
            ...
            this.slots.push({
                ...
                readTypesBuffer: makeBuffer(
                    device,
                    typesOutCapacity,
                    GPU_BUFFER_USAGE_COPY_DST | GPU_BUFFER_USAGE_MAP_READ,
                ),
                readMasksBuffer: makeBuffer(
                    device,
                    masksOutCapacity,
                    GPU_BUFFER_USAGE_COPY_DST | GPU_BUFFER_USAGE_MAP_READ,
                ),
                typesOutBuffer: makeBuffer(
                    device,
                    typesOutCapacity,
                    GPU_BUFFER_USAGE_STORAGE | GPU_BUFFER_USAGE_COPY_DST | GPU_BUFFER_USAGE_COPY_SRC,
                ),
                masksOutBuffer: makeBuffer(
                    device,
                    masksOutCapacity,
                    GPU_BUFFER_USAGE_STORAGE | GPU_BUFFER_USAGE_COPY_DST | GPU_BUFFER_USAGE_COPY_SRC,
                ),
    ...
    private replaceBuffer(slot: BufferSlot, key: keyof BufferSlot, size: number, usage: number): void {
        (slot[key] as GPUBuffer).destroy();
        (slot as unknown as Record<string, GPUBuffer>)[key] = makeBuffer(this.device, size, usage);
    }

    private ensureSlotBuffers(slot: BufferSlot, numWords: number): void {
    ...
    private ensureSlotOutputBuffers(slot: BufferSlot, innerBlocks: number): void {
    ...
    uploadSrc(src: SparseVoxelGrid): void {
        this.releaseSrc();
        this.srcTypesBuffer = makeBuffer(
            this.device,
            src.types.byteLength,
            GPU_BUFFER_USAGE_STORAGE | GPU_BUFFER_USAGE_COPY_DST,
        );
        this.device.queue.writeBuffer(
            this.srcTypesBuffer,
            0,
            src.types.buffer as ArrayBuffer,
            src.types.byteOffset,
            src.types.byteLength,
        );
        const keysU32 = new Uint32Array(src.masks.keys.buffer, src.masks.keys.byteOffset, src.masks.keys.length);
        this.srcKeysBuffer = makeBuffer(
            this.device,
            keysU32.byteLength,
            GPU_BUFFER_USAGE_STORAGE | GPU_BUFFER_USAGE_COPY_DST,
        );
        this.srcLoBuffer = makeBuffer(
            this.device,
            src.masks.lo.byteLength,
            GPU_BUFFER_USAGE_STORAGE | GPU_BUFFER_USAGE_COPY_DST,
        );
        this.srcHiBuffer = makeBuffer(
            this.device,
            src.masks.hi.byteLength,
            GPU_BUFFER_USAGE_STORAGE | GPU_BUFFER_USAGE_COPY_DST,
        );
        this.device.queue.writeBuffer(
            this.srcKeysBuffer,
            0,
            keysU32.buffer as ArrayBuffer,
            keysU32.byteOffset,
            keysU32.byteLength,
        );
        this.device.queue.writeBuffer(
            this.srcLoBuffer,
            0,
            src.masks.lo.buffer as ArrayBuffer,
            src.masks.lo.byteOffset,
            src.masks.lo.byteLength,
        );
        this.device.queue.writeBuffer(
            this.srcHiBuffer,
            0,
            src.masks.hi.buffer as ArrayBuffer,
            src.masks.hi.byteOffset,
            src.masks.hi.byteLength,
        );
        this.srcMeta = {
            nbx: src.nbx,
    ...
    submitChunkSparse(
        ...
        function makeUniform(values: Uint32Array): GPUBuffer {
...

        {
            ...
            dispatch(
                this.dilateXPipeline,
                slot.bufferA,
                slot.bufferB,
                new Uint32Array([numXWords, outerNy, outerNz, halfExtentXZ]),
                Math.ceil(numXWords / 8),
                Math.ceil(outerNy / 4),
                Math.ceil(outerNz / 8),
            );
            dispatch(
                this.dilateYZPipeline,
                slot.bufferB,
                slot.bufferA,
                new Uint32Array([numXWords, outerNy, outerNz, halfExtentXZ, numXWords * outerNy, outerNz]),
                Math.ceil(numXWords / 8),
                Math.ceil(outerNy / 4),
                Math.ceil(outerNz / 8),
            );
            dispatch(
                ...
                slot.bufferA,
                slot.bufferB,
                new Uint32Array([numXWords, outerNy, outerNz, halfExtentY, numXWords, outerNy]),
                Math.ceil(numXWords / 8),
                Math.ceil(outerNy / 4),
                Math.ceil(outerNz / 8),
            );
            encoder.clearBuffer(slot.typesOutBuffer, 0, typesOutWords * 4);
            const compactUniformBuffer = makeUniform(
                new Uint32Array([haloBx, haloBy, haloBz, numXWords, innerBx, innerBy, innerBz, outerBy]),
            );
...
export async function gpuDilate3(
    src: SparseVoxelGrid,
    halfExtentXZ: number,
    halfExtentY: number,
        ...
        for (let cz = 0; cz < src.nz; cz += innerStep) {
            for (let cy = 0; cy < src.ny; cy += innerStep) {
                for (let cx = 0; cx < src.nx; cx += innerStep) {
                    ...
                    const { types, masks } = gpu.submitChunkSparse(
                        currentSlot,
                        minBx,
                        minBy,
                        minBz,
