...
function forEachNonEmptyBlock(grid: SparseVoxelGrid, fn: (blockIdx: number) => void): void {
 ...
 */
function sparseDilate3(src: SparseVoxelGrid, halfExtentXZ: number, halfExtentY: number): SparseVoxelGrid {
 ...
 */
function sparseOrGrids(a: SparseVoxelGrid, b: SparseVoxelGrid, consumeA = false): SparseVoxelGrid {
...
function twoLevelBFS(
    ...
    function enqueueVoxel(ix: number, iy: number, iz: number): void {
        if (vqSize >= vqCap) {
            growVoxelQueue();
        }
        vqIx[vqTail] = ix;
    ...
    function enqueueFaceVoxels(nBlockIdx: number, face: number, nBx: number, nBy: number, nBz: number): void {
    ...
    function processBlock(blockIdx: number): void {
        const bx = blockIdx % nbx;
    ...
    function tryEnqueueVoxel(ix: number, iy: number, iz: number): void {
 ...
 */
export async function fillExterior(
    ...
    const faceVoxelSeeds: Array<{ ix: number; iy: number; iz: number }> = [];
    function seedBoundaryBlock(blockIdx: number, bx: number, by: number, bz: number, face: number): void {
...
export async function carve(
    ...
    if (blocked.getVoxel(seedIx, seedIy, seedIz)) {
        ...
        if (!found) {
            logger.warn(
                `carve: seed (${seed.x}, ${seed.y}, ${seed.z}) blocked after dilation, no free cell within ${maxRadius} voxels, skipping`,
            );
            return { grid, gridBounds: cloneBounds(gridBounds) };
        }
        useSeedIx = found.ix;
