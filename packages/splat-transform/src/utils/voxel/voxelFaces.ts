 ...
 */
function voxelFaces(grid: SparseVoxelGrid, gridBounds: Bounds, voxelResolution: number): Mesh {
    ...
    function addFace(bucket: number, p: number, u: number, v: number): void {
        if (faceLen === faceCap) {
            faceCap *= 2;
    ...
    function blockTypeAt(bx: number, by: number, bz: number): number {
    ...
    function isVoxelSetLocal(lo: number, hi: number, lx: number, ly: number, lz: number): boolean {
        const bitIdx = lx + (ly << 2) + (lz << 4);
        return bitIdx < 32 ? ((lo >>> bitIdx) & 1) !== 0 : ((hi >>> (bitIdx - 32)) & 1) !== 0;
    }

    function isVoxelSetGlobal(ix: number, iy: number, iz: number): boolean {
    ...
    function addVoxelFace(ix: number, iy: number, iz: number, bucket: number): void {
        switch (bucket) {
            case 0:
                addFace(0, ix, iy, iz);
                break; // -X
    ...
    function processSolidBlock(bx: number, by: number, bz: number): void {
        const x0 = bx << 2;
        const y0 = by << 2;
        const z0 = bz << 2;

        function emitX(bucket: number, neighborBlockType: number, ix: number, nx2: number): void {
        ...
        function emitY(bucket: number, neighborBlockType: number, iy: number, ny2: number): void {
            if (neighborBlockType === BLOCK_SOLID) {
                return;
            }
            for (let lz = 0; lz < 4; lz++) {
        ...
        function emitZ(bucket: number, neighborBlockType: number, iz: number, nz2: number): void {
            if (neighborBlockType === BLOCK_SOLID) {
                return;
            }
            for (let ly = 0; ly < 4; ly++) {
    ...
    function processMixedBlock(blockIdx: number, bx: number, by: number, bz: number): void {
        const s = masks.slot(blockIdx);
        const lo = masks.lo[s];
        const hi = masks.hi[s];
        const x0 = bx << 2;
    ...
    let rectCap = 1024;
    let rectLen = 0;
    let rectBucket: Int32Array<ArrayBufferLike> = new Int32Array(rectCap);
    let rectP: Int32Array<ArrayBufferLike> = new Int32Array(rectCap);
    let rectU0: Int32Array<ArrayBufferLike> = new Int32Array(rectCap);
    let rectV0: Int32Array<ArrayBufferLike> = new Int32Array(rectCap);
    let rectU1: Int32Array<ArrayBufferLike> = new Int32Array(rectCap);
    let rectV1: Int32Array<ArrayBufferLike> = new Int32Array(rectCap);

    function addRect(bucket: number, p: number, u0: number, v0: number, u1: number, v1: number): void {
        if (rectLen === rectCap) {
            rectCap *= 2;
            function grow(src: Int32Array<ArrayBufferLike>): Int32Array<ArrayBufferLike> {
    ...
    function decodeGroup(key: number): { bucket: number; p: number } {
    ...
    while (groupStart < keys.length) {
        ...
        function uvKeyOf(u: number, v: number): number {
            return u * coordStride + v;
        }
    ...
    function globalPoint(axis: number, p: number, u: number, v: number): [number, number, number] {
        if (axis === 0) {
            return [p, u, v];
        }
        if (axis === 1) {
    ...
    function lineKey(varAxis: number, x: number, y: number, z: number): number {
        if (varAxis === 0) {
    ...
    function addLinePoint(key: number, value: number): void {
    ...
    function addLineSegment(x0: number, y0: number, z0: number, x1: number, y1: number, z1: number): void {
        if (x0 !== x1) {
    ...
    function addPosition(x: number, y: number, z: number): number {
        if (posLen + 3 > posCap) {
            posCap *= 2;
    ...
    function vertexKey(x: number, y: number, z: number): number {
    ...
    function resetPerimeter(): void {
        perimeterLen = 0;
    }

    function localUv(axis: number, x: number, y: number, z: number): [number, number] {
        if (axis === 0) {
            return [y, z];
        }
        if (axis === 1) {
    ...
    function addPerimeterVertex(v: number, u: number, pv: number): void {
    ...
    function addEdgeVertices(
        ...
        function emitPoint(x: number, y: number, z: number): void {
    ...
    function isConvexEar(prev: number, curr: number, next: number): boolean {
    ...
    function isNonDegenerateTri(a: number, b: number, c: number): boolean {
    ...
    function triangulatePerimeter(useLocalCcw: boolean): void {
        if (perimeterLen < 3) {
            return;
        }
