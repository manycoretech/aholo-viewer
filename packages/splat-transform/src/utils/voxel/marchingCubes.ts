...
function marchingCubes(
    ...
    function isOccupiedLocal(cx: number, cy: number, cz: number): boolean {
        if (cx < 0 || cy < 0 || cz < 0) {
            return false;
        }
    ...
    function addPosition(px: number, py: number, pz: number): number {
        if (posLen + 3 > posCap) {
            posCap *= 2;
    ...
    function ensureIndexCapacity(additional: number): void {
    ...
    function appendTri(a: number, b: number, c: number): void {
    ...
    function splitLineKey(varAxis: number, x2: number, y2: number, z2: number): number {
    ...
    function addSplitLinePoint(key: number, value: number): void {
        if (!splitLinePoints) {
            return;
        }
    ...
    function addSplitPointForVertex(vx: number, vy: number, vz: number, axis: number): void {
    ...
    function getVertex(vx: number, vy: number, vz: number, axis: number): number {
    ...
    function addFaceCell(bucket: number, p: number, u: number, v: number): void {
    ...
    function addDiagCell(bucket: number, plane: number, u: number, e: number): void {
    ...
    function collectFlatFace(cubeIndex: number, vx: number, vy: number, vz: number): boolean {
        if (!mergeFlatFaces) {
            return false;
        }
    ...
    function scaledFacePoint(axis: number, p: number, u: number, v: number): [number, number, number] {
        if (axis === 0) {
    ...
    function diagBucket(axisA: number, axisB: number, signA: number, signB: number): number {
        const pair = axisA === 0 ? (axisB === 1 ? 0 : 1) : 2;
        const signBits = (signA > 0 ? 1 : 0) | (signB > 0 ? 2 : 0);
        return pair * 4 + signBits;
    }

    function decodeDiagBucket(bucket: number): {
        axisA: number;
        axisB: number;
        axisE: number;
        signA: number;
    ...
    function coordByAxis(x: number, y: number, z: number, axis: number): number {
        if (axis === 0) {
            return x;
        }
        if (axis === 1) {
    ...
    function diagPoint(bucket: number, plane: number, u: number, e: number): [number, number, number] {
    ...
    function edgeScaledPoint(edge: number, vx: number, vy: number, vz: number, out: Int32Array, offset: number): void {
        const x = vx * 2;
        const y = vy * 2;
        const z = vz * 2;
        switch (edge) {
    ...
    function samePoint(src: Int32Array, a: number, b: number): boolean {
        return src[a] === src[b] && src[a + 1] === src[b + 1] && src[a + 2] === src[b + 2];
    }

    function pointInUnique(x: number, y: number, z: number, uniqueCount: number): boolean {
    ...
    function collectDiagFaces(triRow: number[], vx: number, vy: number, vz: number): number {
        if (!mergeFlatFaces) {
            return 0;
        }
    ...
    function getScaledVertex(x2: number, y2: number, z2: number): number {
        if ((x2 & 1) !== 0) {
    ...
    function localFaceUv(axis: number, x2: number, y2: number, z2: number): [number, number] {
        if (axis === 0) {
            return [y2, z2];
        }
        if (axis === 1) {
    ...
    function addPerimeterVertex(vertex: number, u: number, v: number): void {
    ...
    function addPerimeterPoint(axis: number, x2: number, y2: number, z2: number): void {
        const [u, v] = localFaceUv(axis, x2, y2, z2);
        addPerimeterVertex(getScaledVertex(x2, y2, z2), u, v);
    }

    function addDiagPerimeterPoint(bucket: number, x2: number, y2: number, z2: number): void {
        const { axisA, axisB, axisE, signA, signB } = decodeDiagBucket(bucket);
    ...
    function addSplitSegment(x0: number, y0: number, z0: number, x1: number, y1: number, z1: number): void {
    ...
    function addSplitEdgeVertices(
        ...
        y1: number,
        z1: number,
        addPoint?: (x2: number, y2: number, z2: number) => void,
    ): void {
        function addDefaultPerimeterPoint(px: number, py: number, pz: number): void {
        ...
        function emitPoint(t: number): void {
            if (varAxis === 0) {
                emit(t, y0, z0);
            } else if (varAxis === 1) {
                emit(x0, t, z0);
    ...
    function appendOrientedTri(a: number, b: number, c: number, useLocalCcw: boolean): void {
        if (useLocalCcw) {
            appendTri(a, b, c);
        } else {
            appendTri(a, c, b);
    ...
    function appendPerimeterTri(a: number, b: number, c: number, useLocalCcw: boolean): void {
    ...
    function triangulateTwoSideChain(
        chainAStart: number,
        chainAEnd: number,
        chainBStart: number,
        chainBEnd: number,
    ...
    function emitFaceRectangle(bucket: number, p: number, u0: number, v0: number, u1: number, v1: number): void {
        const axis = bucket >> 1;
    ...
    function emitDiagRectangle(bucket: number, plane: number, u0: number, e0: number, u1: number, e1: number): void {
        ...
        function addPoint(x2: number, y2: number, z2: number): void {
    ...
    function flushFaceCells(): void {
        ...
        function addRect(bucket: number, p: number, u0: number, v0: number, u1: number, v1: number): void {
            if (rectLen === rectCap) {
                rectCap *= 2;
                function grow(src: Int32Array<ArrayBuffer>): Int32Array<ArrayBuffer> {
        ...
        function addDiagRect(bucket: number, plane: number, u0: number, e0: number, u1: number, e1: number): void {
        ...
        function decodeGroup(key: number): { bucket: number; pOff: number } {
        ...
        function decodeUvKey(key: number): number {
        ...
        while (start < keys.length) {
            ...
            function lookup(uvKey: number): number {
                let h = hash(uvKey);
                while (true) {
                    const k = hKeys[h];
                    if (k === uvKey) {
            ...
            function uvKeyOf(uOff: number, vOff: number): number {
                return uOff * faceCoordStride + vOff;
            }
            const p = pOff - 1;
        ...
        if (diagCellLen > 0) {
            ...
            function decodeDiagKey(key: number): {
                bucket: number;
                plane: number;
                u: number;
                e: number;
