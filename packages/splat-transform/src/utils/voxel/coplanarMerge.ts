 ...
 */
function coplanarMerge(mesh: Mesh, voxelResolution: number): Mesh {
    ...
    function planeEps(da: number, db: number): number {
    ...
    function ensureTriCap() {
        ...
        function growF32(src: Float32Array): Float32Array {
            const out = new Float32Array(newCap);
            out.set(src);
            return out;
        }
        function growF64(src: Float64Array): Float64Array {
    ...
    function addTriToVert(v: number, tri: number) {
        const n = allocNode();
        poolTri[n] = tri;
    ...
    function removeTriFromVert(v: number, tri: number): boolean {
        let prev = -1;
        let cur = vertHead[v];
        while (cur !== -1) {
            if (poolTri[cur] === tri) {
    ...
    function buildBasis(nx: number, ny: number, nz: number): [number, number, number, number, number, number] {
    ...
    function growFanScratch(need: number) {
    ...
    // oversized.
    function extractFan(v: number): number {
        let k = 0;
    ...
    function growEarInternalScratch(n: number) {
    ...
    function earClip(px: Float64Array, py: Float64Array, n: number, out: Int32Array, outOffset: number): number {
        if (n < 3) {
            return -1;
        }
        if (n === 3) {
        ...
        function inTri(p: number, a: number, b: number, c: number): boolean {
            const x = px[p],
                y = py[p];
        ...
        function isEar(a: number, b: number, c: number): boolean {
            if (!isConvex(a, b, c)) {
                return false;
            }
            let p = next[c];
    ...
    function ensureArcScratch(g: number, polySize: number) {
