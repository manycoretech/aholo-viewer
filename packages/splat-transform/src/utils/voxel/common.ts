...
export function decodeMorton3(m: number): [number, number, number] {
    let x = 0,
        y = 0,
        z = 0;
    let bit = 1;
...
export function alignGridBounds(bounds: Bounds, voxelResolution: number): Bounds {
...
export function checkVoxelGridCapacity(gridBounds: Bounds, voxelResolution: number): void {
    const blockSize = LEAF_SIZE * voxelResolution;
    ...
    if (totalBlocks > MAX_VOXEL_BLOCK_COUNT_INT32) {
        throw new Error(
            `voxel grid block count ${totalBlocks} exceeds limit ${MAX_VOXEL_BLOCK_COUNT_INT32} ` +
...
export function extentsFromQuatScale(
    sx: number,
    sy: number,
    sz: number,
    qx: number,
    ...
    qw: number,
    opacity?: number,
    opacityThreshold = ALPHA_THRESHOLD,
) {
    let extend = 3;
...
function quickselect(axisData: Float32Array, idx: Uint32Array, k: number): number {
    function valAt(p: number) {
...
export class GaussianBVH {
    ...
    private computeBounds(indices: Uint32Array): BVHBounds {
        ...
        for (let i = 0; i < indices.length; i++) {
            const idx = indices[i];
            const ex = this.extents[idx * 3],
                ey = this.extents[idx * 3 + 1],
                ez = this.extents[idx * 3 + 2];
            const gx = this.x[idx],
                gy = this.y[idx],
                gz = this.z[idx];
            if (gx - ex < minX) {
    ...
    private buildNode(indices: Uint32Array): BVHNode {
...
function growFloat64(src: Float64Array<ArrayBufferLike>, newCap: number): Float64Array<ArrayBufferLike> {
    const grown = new Float64Array(newCap);
    grown.set(src);
    return grown;
}

function growUint32(src: Uint32Array<ArrayBufferLike>, newCap: number): Uint32Array<ArrayBufferLike> {
...
export class BlockMaskBuffer {
    private solidIdx: Float64Array<ArrayBufferLike> = new Float64Array(0);
    private solidCountValue = 0;
    private solidCap = 0;

    private mixedIdx: Float64Array<ArrayBufferLike> = new Float64Array(0);
    private mixedCountValue = 0;
    private mixedCap = 0;
    private mixedMasks: Uint32Array<ArrayBufferLike> = new Uint32Array(0);
    ...
    getMixedBlocks(): { blockIdx: Float64Array<ArrayBufferLike>; masks: Uint32Array<ArrayBufferLike> } {
        return {
...
function getBlockTypeWordIndex(blockIdx: number): number {
    return Math.floor(blockIdx / BLOCKS_PER_WORD);
}

function getBlockTypeBitShift(blockIdx: number): number {
    return (blockIdx % BLOCKS_PER_WORD) << 1;
}

function getBlockTypeWordCount(totalBlocks: number): number {
    return Math.ceil(totalBlocks / BLOCKS_PER_WORD);
}

function readBlockType(types: Uint32Array, blockIdx: number): number {
    const word = getBlockTypeWordIndex(blockIdx);
    const shift = getBlockTypeBitShift(blockIdx);
    return (types[word] >>> shift) & TYPE_MASK;
}

function writeBlockType(types: Uint32Array, blockIdx: number, blockType: number): void {
    const word = getBlockTypeWordIndex(blockIdx);
    const shift = getBlockTypeBitShift(blockIdx);
...
class SparseVoxelGrid {
    ...
    getOccupiedBlockBounds(
        onProgress?: (done: number, total: number) => void,
    ): { minBx: number; minBy: number; minBz: number; maxBx: number; maxBy: number; maxBz: number } | null {
        const { nbx, nby } = this;
        const totalBlocks = nbx * nby * this.nbz;
        let minBx = nbx,
            minBy = nby,
    ...
    getNavigableBlockBounds(
        ...
        const lastNonEmptyMask =
            lastLanes >= BLOCKS_PER_WORD ? EVEN_BITS : (((1 << (lastLanes * 2)) - 1) >>> 0) & EVEN_BITS;
        let minBx = nbx,
            minBy = nby,
            minBz = this.nbz;
        let maxBx = -1,
...
export class SparseOctree24BitOverflowError extends Error {
    ...
    constructor(kind: 'node' | 'mixed-leaf', actual: number, limit: number) {
        super(
            `Sparse octree ${kind} count (${actual}) exceeds the Laine-Karras 24-bit baseOffset limit (${limit}). Reduce the grid size or split the scene.`,
        );
...
function sortMixedByMorton(mortons: Float64Array, masks: Uint32Array, n = mortons.length): void {
    if (n <= 1) {
        return;
    }
...
function pushInteriorWave(wave: InteriorWave, pos: number, li: number, ii: number): void {
    ...
    while (waveLi.length > 0) {
        ...
        for (let j = 0; j < intPos.length; j++) {
            ...
            if (myLi === 0) {
                ...
                while (true) {
                    const sM =
                        sIdx < nSolid && solidStream[sIdx] < childMortonEnd
                            ? solidStream[sIdx]
                            : Number.POSITIVE_INFINITY;
                    const mM =
                        mIdx < nMixed && mixedStream[mIdx] < childMortonEnd
                            ? mixedStream[mIdx]
                            : Number.POSITIVE_INFINITY;
...
function flattenDenseLevels(
    ...
    function appendNode(value: number): number {
    ...
    function appendMixedLeaf(blockIdx: number): void {
        const leafDataIndex = leafDataLen >> 1;
        if (leafDataIndex > MAX_24BIT_OFFSET) {
            throw new SparseOctree24BitOverflowError('mixed-leaf', leafDataIndex + 1, MAX_24BIT_OFFSET + 1);
        }
    ...
    function appendDenseNode(li: number, idx: number, wave: InteriorWave): void {
 ...
 */
export function buildSparseOctree(
    grid: SparseVoxelGrid,
    gridBounds: Bounds,
    sceneBounds: Bounds,
    voxelResolution: number,
