...
export function buildCollisionMesh(
    ...
    if (shape === 'faces') {
        ...
        logger.info(
            `collision mesh faces: ${mesh.positions.length / 3} vertices, ${mesh.indices.length / 3} triangles`,
        );
    } else {
        const preMerged = marchingCubes(grid, gridBounds, voxelResolution, { mergeFlatFaces: true });
        logger.info(
            `collision mesh pre-merged: ${preMerged.positions.length / 3} vertices, ${preMerged.indices.length / 3} triangles`,
        );
        if (preMerged.indices.length < 3) {
            ...
            logger.info(
                `collision mesh merged: ${mesh.positions.length / 3} vertices, ${mesh.indices.length / 3} triangles`,
            );
